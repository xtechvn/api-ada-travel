    ...
    public class VinWonderB2CController : ControllerBase
    {
        ...
        public async Task<ActionResult> GetTicketOffers(string token)
        {
            ...
            //     token = CommonHelper.Encode(data_product, configuration["DataBaseConfig:key_api:b2c"]);
                ...
                int supplierId = item["SupplierId"]?.ToObject<int>() ?? 0;
                DateTime visitDate = item["VisitDate"]?.ToObject<DateTime>() ?? DateTime.Today;
                ...
                int? categoryId = item["CategoryId"]?.ToObject<int?>();
                int? ticketTypeId = item["TicketTypeId"]?.ToObject<int?>();
                int? playZoneId = item["PlayZoneId"]?.ToObject<int?>();
                int? productId = item["ProductId"]?.ToObject<int?>();
