            ...
            try
            {
                ...
                if (data.bookings != null && data.bookings.Count > 0)
                {
                    ...
                    foreach (var item in data.bookings)
                    {
                        foreach (var routeInfos in item.routeInfos)
                        {
                            ...
                            var FlyBookingDetail = new FlyBookingDetailViewModel
                            {
                                ...
                                ExpiryDate = (item.timeLimit==""|| item.recordStatus== "ERROR")?DateTime.Now.AddHours(6): DateTime.ParseExact(item.timeLimit.Replace("/","-"), new[] { "dd-MM-yyyy HH:mm", "dd-MM-yyyy HH:mm:ss" }, null),
                                OthersAmount = 0,
                                Adgcommission = 0,
                                SupplierId = 0,
                                Difference = 0,
                                ...
                                FareAdt = item.routeInfos.Count > 1 ? item.fareData.fareADT/2 - (item.fareData.issueFee / ( data.order.numberOfAdult + data.order.numberOfChild)/2) : item.fareData.fareADT - item.fareData.issueFee / (data.order.numberOfAdult + data.order.numberOfChild),
                                TaxAdt = item.routeInfos.Count > 1 ? item.fareData.taxADT/2: item.fareData.taxADT,
                                FeeAdt = item.routeInfos.Count > 1 ? item.fareData.vatADT/2: item.fareData.vatADT,
                                FareChd = item.routeInfos.Count > 1 ? item.fareData.fareCHD / 2 - (item.fareData.issueFee / ( data.order.numberOfAdult + data.order.numberOfChild)/2) : item.fareData.fareCHD - item.fareData.issueFee / (data.order.numberOfAdult + data.order.numberOfChild),
                                TaxChd = item.routeInfos.Count > 1 ? item.fareData.taxCHD / 2 : item.fareData.taxCHD,
                                FeeChd = item.routeInfos.Count > 1 ? item.fareData.vatCHD / 2 : item.fareData.vatCHD,
                                FareInf = item.routeInfos.Count > 1 ? item.fareData.fareINF / 2 : item.fareData.fareINF,
                                TaxInf = item.routeInfos.Count > 1 ? item.fareData.taxINF / 2 : item.fareData.taxINF,
                                FeeInf = item.routeInfos.Count > 1 ? item.fareData.fareINF / 2 : item.fareData.fareINF,
                                ServiceFeeAdt = 0,
                                ServiceFeeChd = 0,
                                ServiceFeeInf = 0,
                                AmountAdt = item.routeInfos.Count > 1 ? (item.fareData.fareADT / 2 + item.fareData.taxADT / 2 + item.fareData.vatADT / 2) * data.order.numberOfAdult : (item.fareData.fareADT + item.fareData.taxADT + item.fareData.vatADT) * data.order.numberOfAdult,
                                AmountChd = item.routeInfos.Count > 1 ? (item.fareData.fareCHD / 2 + item.fareData.taxCHD / 2 + item.fareData.vatCHD / 2) * data.order.numberOfChild : (item.fareData.fareCHD + item.fareData.taxCHD + item.fareData.vatCHD) * data.order.numberOfChild,
                                AmountInf = item.routeInfos.Count > 1 ? (item.fareData.fareINF / 2 + item.fareData.taxINF / 2 + item.fareData.vatINF / 2) * data.order.numberOfInfant: (item.fareData.fareINF  + item.fareData.taxINF+ item.fareData.vatINF) * data.order.numberOfInfant,
                                TotalDiscount = 0,
                                TotalBaggageFee = 0,
                                TotalCommission = 0,
                                ...
                                UpdatedBy = id_bot,
                                SalerId = user.Id,
                                Price = item.routeInfos.Count > 1 ? (item.fareData.fareADT + item.fareData.taxADT + item.fareData.vatADT)/2 * data.order.numberOfAdult + (item.fareData.fareCHD + item.fareData.taxCHD + item.fareData.vatCHD)/2 * data.order.numberOfChild + (item.fareData.fareINF + item.fareData.taxINF + item.fareData.vatINF)/2 * data.order.numberOfInfant + item.fareData.otherFee : (item.fareData.fareADT + item.fareData.taxADT + item.fareData.vatADT) * data.order.numberOfAdult + (item.fareData.fareCHD + item.fareData.taxCHD + item.fareData.vatCHD) * data.order.numberOfChild + (item.fareData.fareINF + item.fareData.taxINF + item.fareData.vatINF) * data.order.numberOfInfant + item.fareData.otherFee,

                                Amount = item.routeInfos.Count > 1 ? (item.fareData.fareADT + item.fareData.taxADT + item.fareData.vatADT)/2 * data.order.numberOfAdult + (item.fareData.fareCHD + item.fareData.taxCHD + item.fareData.vatCHD)/2 * data.order.numberOfChild + (item.fareData.fareINF + item.fareData.taxINF + item.fareData.vatINF)/2 * data.order.numberOfInfant + item.fareData.otherFee:(item.fareData.fareADT + item.fareData.taxADT + item.fareData.vatADT) * data.order.numberOfAdult + (item.fareData.fareCHD + item.fareData.taxCHD + item.fareData.vatCHD) * data.order.numberOfChild + (item.fareData.fareINF + item.fareData.taxINF + item.fareData.vatINF) * data.order.numberOfInfant + item.fareData.otherFee,
                                TotalNetPrice = item.routeInfos.Count > 1 ? (item.fareData.fareADT + item.fareData.taxADT + item.fareData.vatADT)/2 * data.order.numberOfAdult + (item.fareData.fareCHD + item.fareData.taxCHD + item.fareData.vatCHD)/2 * data.order.numberOfChild + (item.fareData.fareINF + item.fareData.taxINF + item.fareData.vatINF)/2 * data.order.numberOfInfant + item.fareData.otherFee : (item.fareData.fareADT + item.fareData.taxADT + item.fareData.vatADT) * data.order.numberOfAdult + (item.fareData.fareCHD + item.fareData.taxCHD + item.fareData.vatCHD) * data.order.numberOfChild + (item.fareData.fareINF + item.fareData.taxINF + item.fareData.vatINF) * data.order.numberOfInfant + item.fareData.otherFee,
                    ...
                    foreach (var item in data.passengers)
                    {
                        ...
                        PassengerViewModel.Gender = item.Title== "MRS"|| item.Title == "MS"? false:true;
